...
4.- retornar un mensaje dedl tipo '"15 ohms"
 */

function sumOfResistors(resistorArray) {
    const resistorsAbs = resistorArray.map((resistor) => Math.abs(resistor));
    const totalResistance = resistorsAbs.reduce((total, current) => total + current, 0);
    return `${totalResistance} ohmd`
}
